...
async function main() {
  ...
  for (const subpath of subpaths) {
    ...
    // `./package.json` is intentionally exposed so tooling can resolve
    // `eslint-config-agent/package.json` (e.g. `require.resolve` to read the
    // version) without hitting ERR_PACKAGE_PATH_NOT_EXPORTED. It is the package
    // manifest, not a flat-config array, so it gets its own assertion.
