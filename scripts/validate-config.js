...
async function validateConfig() {
  ...
  try {
        ...
        if (result) {
          ...
          if (result.messages.length > 0) {
            result.messages.forEach(msg => {
              ...
              console.log(
                `   ${level} Line ${msg.line}: ${msg.message} (${msg.ruleId})`
              )
            })
          }
          ...
          if (file === 'test/invalid.tsx') {
            const hasOptionalChainingError = result.messages.some(
              m =>
                m.ruleId === 'no-restricted-syntax' &&
                m.message.includes('Optional chaining')
            )
            const hasNullishCoalescingError = result.messages.some(
              m =>
                m.ruleId === 'no-restricted-syntax' &&
                m.message.toLowerCase().includes('nullish coalescing')
            )
            if (!hasOptionalChainingError) {
              console.log(
                '   ⚠️  Optional chaining rule may need adjustment (not critical for basic functionality)'
              )
            }
            if (!hasNullishCoalescingError) {
              console.log(
                '   ⚠️  Nullish coalescing rule may need adjustment (not critical for basic functionality)'
              )
            }
          }

          if (file === 'test/long-function.tsx') {
            const hasMaxLinesWarning = result.messages.some(
              m => m.ruleId === 'max-lines-per-function'
            )

            if (!hasMaxLinesWarning) {
              console.log(
                '   ❌ Expected max-lines-per-function warning not found'
              )
              allPassed = false
            }
          }
          ...
          if (file === 'test/jsx-extension-test.js') {
            const hasJsxExtensionError = result.messages.some(
              m => m.ruleId === 'react/jsx-filename-extension'
            )

            if (!hasJsxExtensionError) {
              console.log(
                '   ❌ Expected jsx-filename-extension error not found'
              )
              allPassed = false
            }
          }
          ...
          if (file === 'test/react-hooks-rules.tsx') {
            const hasHooksErrors = result.messages.some(
              m => m.ruleId && m.ruleId.startsWith('react-hooks/')
            )

            if (!hasHooksErrors) {
              console.log(
                '   ⚠️  Expected react-hooks errors not found (may need adjustment)'
              )
            } else {
          ...
          if (
            ...
            if (result.errorCount > 5) {
              console.log(
                `   ⚠️  High error count (${result.errorCount}) - may indicate config issues`
              )
            }
          }
        }
    ...
    if (!Array.isArray(config)) {
      ...
      console.log(
        `   ✅ Config exports array with ${config.length} configurations`
      )
    }
